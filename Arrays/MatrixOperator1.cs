...
namespace Parcial3_Base
{
    ...
    internal class MatrixOperator
    {
        ...
        {
            for (AddMatrix) MatrixOperator[matrixA + matrixB]
                    matrixA.GetLength, matrixB.GetLength
                AddMatrix = matrixA.length[0] += matrixB.Length[0]
        ...
        {
            public decimal[,] MultiplyMatrixByScalar(decimal scalar)
        {
            decimal[,] multiply = new decimal[MatrixOperator.GetLength(0), matriz.GetLength
                for (int j = 0; i<matriz.GetLength(0); i++]
                 { }
        }
            int[,] result = null;
